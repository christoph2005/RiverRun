...
function sceneOpt:createScene( event )
	...
	-- create a widget button (which will loads level1.lua on release)
	profBtn = widget.newButton{
		label="          ",
		labelColor = { default={255}, over={128} },
		defaultFile="Images/Options/profile.gif",
		overFile="Images/Options/profile-over.gif",
		width=200, height=75,
		onRelease = soundBtnRelease	-- event listener function
	}
	profBtn:setReferencePoint( display.CenterReferencePoint )
	profBtn.x = display.contentWidth*0.5
	profBtn.y = display.contentHeight - 62
...
function sceneOpt:destroyScene( event )
	...
	if profBtn then
		profBtn:removeSelf()	-- widgets must be manually removed
		profBtn = nil
	end
